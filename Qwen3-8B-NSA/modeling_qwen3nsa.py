...
class Qwen3NSAAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3NSAConfig, layer_idx: int):
        ...
        self.nsa_intra_block_pe = torch.nn.Parameter(
            torch.zeros(config.num_key_value_heads, config.nsa_kernel_size, self.head_dim)
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if hidden_states.shape[1]>1:
            ...
            step=1
        query_states = self.q_norm(self.q_proj(hidden_states).view(hidden_states.shape[0], hidden_states.shape[1], self.config.num_attention_heads, self.head_dim)).transpose(1, 2)
        key_states = self.k_norm(self.k_proj(hidden_states).view(hidden_states.shape[0], hidden_states.shape[1], self.config.num_key_value_heads, self.head_dim)).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(hidden_states.shape[0], hidden_states.shape[1], self.config.num_key_value_heads, self.head_dim).transpose(1, 2)
        ...
        if step==0:
            ...
            compressed_key_states = cuseqlen2batch(compressed_key_states, compressed_cu_seqlens, self.config.num_key_value_heads, self.head_dim)
            compressed_value_states = cuseqlen2batch(compressed_value_states, compressed_cu_seqlens, self.config.num_key_value_heads, self.head_dim)
            key_states_batched = cuseqlen2batch(key_states, cu_seqlens, self.config.num_key_value_heads, self.head_dim)
            value_states_batched = cuseqlen2batch(value_states, cu_seqlens, self.config.num_key_value_heads, self.head_dim)
            ...
            sliding_attn_output = flash_attn_varlen_func(
                ...
                key_states,
                value_states,
                torch.arange(0, len(cu_seqlens), dtype=torch.int32).cuda(),
                cu_seqlens.to(torch.int32),
                1,
                seqlens.max().item(),
                causal=True,
...
class Qwen3NSAForCausalLM(Qwen3NSAPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, Qwen3NSAForCausalLM

        >>> model = Qwen3NSAForCausalLM.from_pretrained("Qwen/Qwen3-8B")
    ...
    def inference(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> # Generate
        >>> generate_ids = model.inference(inputs.input_ids, max_length=30)
        >>> tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
        "Hey, are you conscious? Can you talk to me?\nI'm not conscious, but I can talk to you."
        ```"""
        outputs: BaseModelOutputWithPast = self.model(
